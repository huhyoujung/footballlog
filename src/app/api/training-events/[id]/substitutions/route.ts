...
// POST /api/training-events/[id]/substitutions - 교체 기록 생성
export async function POST(
  ...
  try {
    ...
    const { quarter, minute, teamSide: rawTeamSide, playerOutId, playerInId } = await req.json();

    if (!quarter || !rawTeamSide) {
    ...
    if (teamSide === "TEAM_A" && (!playerOutId || !playerInId)) {
      return NextResponse.json({ error: "우리팀 교체는 두 선수를 선택해야 합니다" }, { status: 400 });
    }
    ...
    console.error("Substitution record error:", error);
