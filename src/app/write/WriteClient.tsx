...
function WritePageContent() {
  ...
  return (
    <div className="min-h-screen bg-white pb-24">
      <PageHeader title={isEditMode ? "운동 일지 수정" : "운동 일지 작성"} left={<BackButton />} />
      ...
      <main className="max-w-2xl mx-auto divide-y divide-gray-100">
        ...
        <div className="px-4 py-5">
          ...
          <MentionTextarea
            ...
            onChange={(value) =>
              setFormData({ ...formData, conditionReason: value })
            }
        ...
        {!isEditMode && (
          <div className="px-4 pt-0 pb-5 flex flex-col items-center !border-t-0">
            <button
              type="button"
              onClick={() => !imagePreview && !compressing && fileInputRef.current?.click()}
              disabled={compressing}
              className="w-32 bg-white rounded-sm p-2 pb-4 cursor-pointer disabled:opacity-50"
            ...
            >
              {imagePreview ? (
                ...
                <div className="w-full aspect-[3/4] bg-team-50 rounded-sm flex flex-col items-center justify-center gap-2">
                  <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-team-500" />
                  <span className="text-xs text-gray-400">압축 중...</span>
                </div>
              ) : (
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-gray-200 z-10">
          <div className="max-w-2xl mx-auto flex justify-center">
            <button
              ...
              disabled={loading}
              className="w-full max-w-xs py-3.5 bg-team-500 text-white rounded-xl font-semibold hover:bg-team-600 transition-all active:scale-[0.97] touch-manipulation disabled:opacity-50"
            >
