# MVP 결과 바텀시트 통합 구현 계획

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 피드/상세에서 각각 사용하던 MVP 결과 바텀시트를 단일 컴포넌트로 통합하고, 공유 이미지에 전체 순위·추천사를 포함하며, 공동 순위를 올바르게 처리한다.

**Architecture:** `MvpResultSheet.tsx`를 전면 재작성해 공유 기능·전체 순위·추천사·공동 순위를 모두 지원하는 단일 컴포넌트로 만든다. pom GET API에서 `eventDate`와 `teamName`을 함께 반환해 컴포넌트가 props 없이 자체적으로 데이터를 수신한다. `PomVoting.tsx`의 내부 `ResultsSheet` 함수는 삭제하고 `MvpResultSheet`로 교체한다.

**Tech Stack:** Next.js App Router, TypeScript, Tailwind CSS v4, Prisma, html-to-image (toPng)

---

### Task 1: pom GET API에 `eventDate`, `teamName` 추가

**Files:**
- Modify: `src/app/api/training-events/[id]/pom/route.ts:20-23`

**Step 1: event 쿼리에 team 포함하도록 수정**

`prisma.trainingEvent.findUnique`의 `select`를 다음으로 교체:
...
**Step 2: 응답에 `eventDate`, `teamName` 추가**

`return NextResponse.json(...)` 블록에 두 필드 추가:
...
```
Expected: 타입 에러 없음 (또는 이 파일과 무관한 에러만)

**Step 4: 커밋**

```bash
git add src/app/api/training-events/\[id\]/pom/route.ts
git commit -m "feat: pom API 응답에 eventDate, teamName 추가"
```

---

### Task 2: `MvpResultSheet.tsx` 전면 재작성

**Files:**
- Modify: `src/components/MvpResultSheet.tsx` (전체 교체)

**Step 1: 파일 전체를 다음 코드로 교체**
      ...
      <div style={{ position: "fixed", left: "-9999px", top: 0 }}>
        ...
        >
          ...
          >
            ...
            {/* 1위 추천사 (단독 1위일 때만 최대 2개, 공동일 때 각 1개) */}
...
```bash
cd /Users/huhyoujung/dev/football-log && npm run build 2>&1 | grep -E "error|Error" | head -20
```
Expected: MvpResultSheet 관련 에러 없음

**Step 3: 커밋**

```bash
git add src/components/MvpResultSheet.tsx
git commit -m "feat: MvpResultSheet 통합 재작성 — 공유, 공동순위, 전체추천사"
```

---

### Task 3: `PomVoting.tsx`에서 `ResultsSheet` 제거 후 `MvpResultSheet` 사용
...
**Step 2: `ResultsSheet` 사용 위치 2곳을 `MvpResultSheet`로 교체**

교체 전 (마감 후 투표 안 한 사용자 경로, 약 line 223):
```tsx
...
**Step 3: `ResultsSheet` 함수 전체 삭제**

`PomVoting.tsx` 파일에서 `/* ─── 결과 바텀시트 ─── */` 주석부터 `ResultsSheet` 함수 끝까지 (약 line 501~868) 삭제.

**Step 4: 더 이상 필요 없는 import 정리**

`PomVoting.tsx` 상단의 다음 import들이 ResultsSheet에서만 쓰였다면 삭제:
- `toPng` from `"html-to-image"` → MvpResultSheet로 이동했으므로 삭제

**Step 5: 빌드 타입 체크**

```bash
cd /Users/huhyoujung/dev/football-log && npm run build 2>&1 | tail -30
```
Expected: 타입 에러 없음
...
git commit -m "refactor: PomVoting ResultsSheet 제거, MvpResultSheet로 통합"
```
...
```bash
cd /Users/huhyoujung/dev/football-log && npm run build 2>&1 | tail -40
```
Expected: `✓ Compiled successfully` 또는 Route 목록만 출력, 빨간 에러 없음

**Step 2: 수동 확인 체크리스트**

로컬 서버(`npm run dev`) 실행 후 확인:
- [ ] 피드에서 트로피 아이콘 클릭 → MVP 바텀시트 열림, 공유 버튼 있음
- [ ] 훈련 상세에서 "결과 보기" → 같은 디자인의 바텀시트 열림
- [ ] 2위 이하 선수들 코멘트 가로 스크롤 동작
- [ ] "이미지로 공유하기" → 이미지 캡처 후 공유/다운로드
- [ ] 공유 이미지에 전체 순위 + 추천사 포함
- [ ] (테스트용) 동점 데이터로 공동 순위 표시 확인

**Step 3: 최종 커밋 (필요 시)**

```bash
git add -A
git commit -m "chore: MVP 바텀시트 통합 완료"
```
