...
**표시 내용** (우선순위 순, 롤링):
1. 팀 운동 공지 (최우선)
   - "📢 {운동명} · MM/DD (요일) HH:MM · {장소}"
2. MVP 메시지 (24시간 이내)
   - "🏆 OOO님이 오늘/어제 MVP였습니다!"
3. **오늘 1등 메시지** (오늘 로그가 정확히 1개일 때만)
   - "🏆 OOO님이 오늘 첫 주자! · HH:MM"
   - 2등이 올리면 자동으로 사라짐 (게이미피케이션)
4. 닦달 메시지 (최근 2시간 이내)
   - "👉 OOO님이 XXX님을 닦달했습니다"
5. 활동 메시지 (상시)
   - "오늘 N명 운동 완료! 🔥"
   - "라커룸이 조용하네요 오늘의 첫 기록을 남겨보세요!"
   - "전원 출석! N명 운동 완료 🎉"
...
**정렬 로직** (게이미피케이션):
- **오늘 사진**: 가장 최근에 올라온 것이 맨 위로 (최신순)
  - 이유: 사람들이 신규 글을 많이 보게 하기 위함
  - 정렬: `createdAt DESC`
- **과거 사진** (오늘이 지나간 후): 가장 먼저 올린 사람이 가장 위에 (등록 시간 오름차순)
  - 이유: 맨날 먼저 올리는 경쟁을 가속시키도록
  - 정렬: `createdAt ASC`
